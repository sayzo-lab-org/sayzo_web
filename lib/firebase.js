...
const maskPhone = (phone = "") => {
  ...
  const visibleDigits = trimmedPhone.slice(-4);
  const maskedSection = trimmedPhone
    .slice(0, -4)
    .replace(/[0-9]/g, "*");
...
const maskEmail = (email = "") => {
  ...
  if (!domainPart) return trimmedEmail;
...
const mapApplicationWithMaskedContact = (docSnap) => {
...
export const updateTaskStatus = async (taskId, status, rejectionReason = null) => {
  console.log("Updating task ID:", taskId, "| New status:", status);
  try {
    if (!taskId) throw new Error("Invalid taskId provided");

    const taskRef = doc(db, "tasks", taskId);
    console.log("Firestore Path:", taskRef.path);
    ...
    console.log("Payload going to Firestore:", updateData);
    ...
    console.log("Successfully updated task status in Firestore");
  } catch (error) {
    console.error("Full updateTaskStatus error object:", error);
...
export const deleteTask = async (taskId) => {
  ...
  try {
    ...
    console.error("Full deleteTask Error object:", error);
...
const logNotificationEvent = (level, message, meta = {}) => {
  ...
  logger(`[notification-queue] ${message}`, meta);
};
...
export const addApplication = async (applicationData) => {
  ...
  try {
    ...
    const docRef = await addDoc(applicationsRef, {
      ...
      // Explicitly persist giverId so Firestore security rules can verify
      // the task giver's ownership when they accept/reject applications.
  ...
  if (queueJob?.recipientId) {
    try {
      ...
      console.warn("addApplication: notification step failed (non-critical):", notifError.message);
    }
  }
...
export const getApplicationsByTask = async (taskId) => {
  try {
    ...
    return snapshot.docs.map(mapApplicationWithMaskedContact);
  } catch (error) {
...
// for debug
    ...
    // console.log("Task giverId:", taskData.giverId);
  ...
  try {
    ...
    console.warn("updateApplicationStatus: notification step failed (non-critical):", notifError.message);
  }
...
export const subscribeToApplicationsByTask = (taskId, callback, onError) => {
  ...
  return onSnapshot(
    q,
    (snapshot) => {
      const applications = snapshot.docs.map(mapApplicationWithMaskedContact);
...
export const getConversationsByUser = async (uid) => {
  try {
    ...
    console.warn("getConversationsByUser:", error.message);
    return [];
  }
};
...
export const getTaskMetricsBase = async (taskId) => {
  try {
    ...
    console.error("getTaskMetricsBase Error:", error);
...
export const getApplicationCount = async (taskId) => {
  try {
    ...
    console.error("getApplicationCount Error:", error);
