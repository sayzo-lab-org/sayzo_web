...
export default function WelcomeScreen() {
  ...
  const goTo = (path) => {
    localStorage.setItem("sayzo_welcome_seen", "true");
    router.replace(path);
  };
    ...
    <div className="fixed inset-0 z-[9998] flex items-center justify-center bg-[#F3F4F6] overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center h-full w-full max-w-7xl px-4 md:px-12">
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="bg-white rounded-t-[50px] lg:rounded-[70px] shadow-[0_10px_40px_rgba(0,0,0,0.03)] w-full lg:max-w-[560px] p-8 md:p-16 lg:p-20 flex flex-col z-10"
        >

          <h1 className="text-[32px] md:text-[40px] lg:text-[44px] font-bold text-gray-900 mb-4 text-center lg:text-left">
            Welcome to SAYZO
          </h1>

          <p className="text-[#9CA3AF] text-[16px] md:text-[18px] lg:text-[19px] leading-relaxed mb-8 lg:mb-14 text-center lg:text-left">
            Connect with trusted people nearby to get your tasks done instantly,
          ...
          <div className="flex flex-col gap-3 md:gap-5">
            ...
            <button
              onClick={() => goTo("/login")}
              className="w-full bg-black text-white py-4 md:py-6 rounded-[20px] md:rounded-[28px] text-[18px] md:text-[20px] font-semibold"
            >
              Log In
            </button>
            ...
            <button
              onClick={() => goTo("/signup")}
              className="w-full bg-[#F8F9FB] text-gray-900 py-4 md:py-6 rounded-[20px] md:rounded-[28px] text-[18px] md:text-[20px] font-semibold"
            >
              Sign Up
            </button>
