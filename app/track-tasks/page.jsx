...
import { ChevronRight, ChevronDown, ChevronLeft, Loader2, Clock, CheckCircle, XCircle, Eye, X, Info, Lock } from 'lucide-react'
      ...
      {isAuthenticated && (
        <div className="max-w-350 mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[30%_70%] gap-6 py-6">
            {/* My Tasks Section */}
            <div className={`bg-white rounded-lg p-4 md:p-6 ${viewMode === 'giver'
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className={`relative w-24 h-8 rounded-full transition-colors duration-200 border ${viewMode === 'giver' ? 'bg-white border-gray-300' : 'bg-black border-black'
                    }`}
                >
                  <div className={`absolute top-0.5 bottom-0.5 px-3 rounded-full shadow-md transition-all duration-200 flex items-center justify-center text-sm font-semibold ${viewMode === 'giver' ? 'left-0.5 bg-[#10A37F] text-white' : 'right-0.5 bg-white text-black'
                    }`}>
              ...
              <div className="flex mb-6 bg-gray-100 rounded-lg">
                <button
                  ...
                  className={`flex-1 py-4 font-semibold text-center transition-colors ${activeTab === 'offline'
                ...
                <button
                  ...
                  className={`flex-1 py-4 font-semibold text-center transition-colors ${activeTab === 'online'
                    ? 'bg-black text-white rounded-lg'
                    : 'text-gray-500'
                    }`}
                >
                    ...
                    {loading ? (
                      ...
                      filteredTasks.map((task) => (
                        <div
                          ...
                          className={`flex items-center justify-between p-5 rounded-lg cursor-pointer transition-colors ${selectedTaskId === task.id
                            ? 'bg-black text-white'
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                            ...
                            <ChevronRight className={`w-5 h-5 flex-shrink-0 ${selectedTaskId === task.id ? 'text-white' : 'text-gray-400'
                              }`} />
                          </div>
                        </div>
                      ))
                      ...
                      filteredDoerApplications.map((application) => {
                        ...
                        return (
                          <div
                            ...
                            className={`flex items-center justify-between p-5 rounded-lg cursor-pointer transition-colors ${selectedApplicationId === application.id
                              ? 'bg-black text-white'
                              : 'bg-gray-50 hover:bg-gray-100 text-gray-700'
                              }`}
                          >
                            <div className="min-w-0 flex-1">
                              ...
                              <span className={`text-xs ${selectedApplicationId === application.id ? 'text-gray-300' : 'text-gray-400'
                                }`}>
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                              ...
                              <ChevronRight className={`w-5 h-5 flex-shrink-0 ${selectedApplicationId === application.id ? 'text-white' : 'text-gray-400'
                                }`} />
                            </div>
                          </div>
                        )
            ...
            <div className={`relative bg-white rounded-lg p-4 md:p-6 ${viewMode === 'giver'
              ? (!selectedTaskId ? 'hidden lg:block' : 'block')
              : (!selectedApplicationId ? 'hidden lg:block' : 'block')
              }`}>
              {/* Mobile Back Button */}
                    ...
                    {!selectedTaskId ? (
                      ...
                      applicants.map((applicant) => {
                        ...
                        return (
                          <div
                            key={applicant.id}
                            className={`bg-gray-50 rounded-lg overflow-hidden transition-all duration-300 ${isExpanded ? '' : 'hover:bg-gray-100'
                              }`}
                          >
                            ...
                            >
                              <div className="min-w-0 flex-1">
                                ...
                                {applicant.status !== APPLICATION_STATUS.PENDING && (
                                  <span className={`text-xs px-2 py-1 rounded-full ${applicant.status === APPLICATION_STATUS.ACCEPTED
                              ...
                              <div className="flex-shrink-0 ml-2">
                                <ChevronDown
                                  className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${isExpanded ? 'rotate-180' : 'rotate-0'
                                    }`}
                                />
                              </div>
                            </div>
                              ...
                              <div className="px-4 pb-4 space-y-4">
                                ...
                                <div className="space-y-2 text-sm">
                                  ...
                                  <div className="flex flex-col gap-2 text-sm">
                                    {/* Mobile row */}
                                    <div className="flex items-center gap-2">
                                      <span className="text-gray-900 w-14 shrink-0">Mobile:</span>
                                    ...
                                    {/* Email row */}
                                    <div className="flex items-center gap-2">
                                      <span className="text-gray-900 w-14 shrink-0">Email:</span>
                                      {applicant._contactRevealed ? (
                                        <span className="text-gray-800 font-medium">
                                    ...
                                    {/* City row */}
                                    <div className="flex items-center gap-2">
                                      <span className="text-gray-900 w-14 shrink-0">City:</span>
...
// ─── ContactInfoTooltip ──────────────────────────────────────────────────────
// Shows a (ⓘ) icon. On hover (desktop) or click (mobile) displays a soft
// tooltip explaining that contact details are hidden until acceptance.
  ...
  return (
    <span ref={ref} className="relative inline-flex items-center">
      <button
        ...
        // Show on hover (desktop) via group; show on click (all)
      ...
      >
        ...
        <span
          ...
          className="pointer-events-none absolute left-1/2 bottom-full mb-2 -translate-x-1/2
            ...
            w-56 p-3 bg-gray-900 text-white text-xs leading-relaxed
            rounded-xl shadow-xl z-50 text-left"
        >
          <span className="font-semibold block mb-1">Contact details are hidden.</span>
          Accept the applicant to reveal their phone number and email for further communication.
          {/* Arrow */}
        ...
        {open && (
          <motion.span
            ...
            className="absolute left-1/2 bottom-full mb-2 -translate-x-1/2
              w-56 p-3 bg-gray-900 text-white text-xs leading-relaxed
              rounded-xl shadow-xl z-50 text-left lg:hidden"
          >
            <span className="font-semibold block mb-1">Contact details are hidden.</span>
            Accept the applicant to reveal their phone number and email for further communication.
            <span className="absolute left-1/2 -translate-x-1/2 top-full w-0 h-0
