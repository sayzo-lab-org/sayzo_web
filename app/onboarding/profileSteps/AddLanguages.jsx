...
export default function AddLanguages({ onNext, onBack, initialData }) {
  const [languages, setLanguages] = useOnboardingStorage(
    "languages",
  ...
  const deleteLang = (index) => setLanguages(languages.filter((_, i) => i !== index));
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <p className="text-zinc-500 font-normal text-sm md:text-base leading-relaxed">
          Communication is the foundation of every successful task. List the languages you speak and
      ...
      {showForm && (
        <LanguageForm addLanguage={addLanguage} close={() => setShowForm(false)} />
      )}
    </motion.div>
  );
}